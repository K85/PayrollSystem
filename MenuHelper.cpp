...
void MenuHelper::menu_add_dataBean_Batch_FromConsole() {
    ...
    while (scanf("%d %s %lf %lf %lf %lf %lf", &id, &name_buf, &baseSalary, &allowance, &socialInsurance, &medicalInsurance, &accumulationFund) != EOF) {
        DataBean dataBean = DataBean(id, string(name_buf), baseSalary, allowance, socialInsurance, medicalInsurance, accumulationFund);
        SystemInterface::getInstance().addDataBean(dataBean);
        printf("添加成功, 请输入下一个\n");
    }
...
void MenuHelper::menu_modify_dataBean() {
        ...
        if (!SystemInterface::getInstance().existDataBean(DataBean(id))) {
            printf("员工号不存在.");
            return;
        }

        printf("请输入修改后的新数据: \n");
        DataBean dataBean = DataBean::readDataBeanFromID(id);
...
void MenuHelper::menu_searchByID_dataBean() {
    ...
    vector<DataBean>::iterator iter = SystemInterface::getInstance().searchDataBean(DataBean(id));
    iter->printDataBean();
}
...
void MenuHelper::menu_searchByName_dataBean() {
    ...
    vector<DataBean>::iterator iter = SystemInterface::getInstance().searchDataBean(DataBean(name));
    iter->printDataBean();
}
