...
using namespace std;
    ...
    string DataBean::getFormatedInfo() {
        ...
        sprintf(buf, "员工号: %d [名称: %s] [实发工资 %.2f] [应发工资 %.2f] [合计扣款 %.2f] [基本工资 %.2f] [津贴 %.2f] [社保 %.2f] [医保 %.2f] [公积金 %.2f]", this->id, this->name.c_str(),this->getSalary(), this->getGrossSalary(), this->getDiscount(), this->baseSalary, this->allowance, this->socialInsurance, this->medicalInsurance, this->accumulationFund);
        return string(buf);
    }
